package anim.bro.com.practice.banner.bannerview.indicator
        ...
        mCheckedBitmap?.let {
            mIndicatorSize?.let {
                if (mCheckedBitmap!!.isMutable && checkCanResize) {
                    ...
                    mCheckedBitmap =
                        Bitmap.createBitmap(mCheckedBitmap!!, 0, 0, width, height, matrix, true)
                }
            }
        ...
        mNormalBitmap?.let {
            mIndicatorSize?.let {
                if (mNormalBitmap!!.isMutable && normalCanResize) {
                    ...
                    val scaleHeight =
                        mIndicatorSize!!.normalHeight.toFloat() / mNormalBitmap!!.height
                    val matrix = Matrix()
                    matrix.postScale(scaleWidth, scaleHeight)
                    mNormalBitmap =
                        Bitmap.createBitmap(mNormalBitmap!!, 0, 0, width, height, matrix, true)
                }
            }
