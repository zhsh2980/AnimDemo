...
public class GestureLineView extends View {
    ...
    private void init() {
        linearGradient = new LinearGradient(0, 0, 0, UIUtils.dip2px(130),
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        localM.setTranslate(0, getMeasuredHeight() - UIUtils.dip2px(130));
        linearGradient.setLocalMatrix(localM);
        paint.setShader(linearGradient);
        path.moveTo(0, UIUtils.dip2px(6));
        path.lineTo(0, getMeasuredHeight());
        path.lineTo(getMeasuredWidth(), getMeasuredHeight());
        path.lineTo(getMeasuredWidth(), UIUtils.dip2px(11));
        path.close();
